...
switch ($queryId) 
{
    ...
    case "1":
		...
		$output = mysqli_query($connect, $query) ? $DeploymentId : mysqli_error($connect);
		echo json_encode($output);
    ...
    case "3":  
		...
		$secElementId = mysqli_real_escape_string($connect, $request->sectorName);
		...
		$output = mysqli_query($connect, $query) ? $secElementId : mysqli_error($connect);
		echo json_encode($output);
